...
MCP server for the VTEX Returns API, providing AI assistants access to VTEX e-commerce APIs.
...
```bash
npx @vtex-mcp/returns-api
```

### Running with HTTP transport

```bash
npx @vtex-mcp/returns-api --transport http --port 3000
```
...
- **returns_create_return_invoice** — Create return invoice for order
- **returns_get_order_with_return_info** — Get order details including return information
