...
describe("jsonSchemaToZod", () => {
  ...
  it("converts string enum", () => {
    expect(
      jsonSchemaToZod({ type: "string", enum: ["active", "inactive"] }),
    ).toBe('z.enum(["active", "inactive"])');
  });

  it("converts string with minLength and maxLength", () => {
    expect(
      jsonSchemaToZod({ type: "string", minLength: 1, maxLength: 100 }),
    ).toBe("z.string().min(1).max(100)");
  });

  it("converts string with pattern", () => {
    expect(
      jsonSchemaToZod({ type: "string", pattern: "^[A-Z]{2}$" }),
    ).toBe("z.string().regex(/^[A-Z]{2}$/)");
  });
  ...
  it("converts array with items", () => {
    expect(
      jsonSchemaToZod({ type: "array", items: { type: "string" } }),
    ).toBe("z.array(z.string())");
  });

  it("applies default values", () => {
    expect(jsonSchemaToZod({ type: "number", default: 1 })).toBe(
      "z.number().default(1)",
    );
  });
