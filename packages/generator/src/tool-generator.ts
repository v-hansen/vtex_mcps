...
export function isPaginatedOperation(op: ParsedOperation): boolean {
  return op.parameters.some((p) => p.in === 'query' && PAGINATION_PARAM_NAMES.has(p.name));
}
...
function buildHandlerCode(operation: ParsedOperation): string {
  ...
  if (hasBody && (method === 'post' || method === 'put' || method === 'patch')) {
    lines.push(
      '    const response = await http.' + method + '(url, params.body' + configStr + ');',
    );
  } else {
 ...
 */
export function operationToTool(operation: ParsedOperation, apiGroupPrefix: string): GeneratedTool {
