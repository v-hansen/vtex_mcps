...
describe('formatMcpError', () => {
  ...
  describe('credential sanitization', () => {
    ...
    it('redacts authToken from error messages', () => {
      ...
      const err = new Error('Token eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9 is invalid');
      const result = formatMcpError(err);
      expect(result.content[0].text).not.toContain('eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9');
      expect(result.content[0].text).toContain('[REDACTED]');
    });
  });
